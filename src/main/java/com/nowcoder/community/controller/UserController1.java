...
import com.nowcoder.community.util.AliyunOssUtil;
...
//    }
    ...
    public void getHeader(@PathVariable("fileName") String fileName, HttpServletResponse response) {
        ...
        try (
                FileInputStream fis = new FileInputStream(fileName);//读取文件得到一个输入流 try()里的资源会自动关闭
                OutputStream os = response.getOutputStream();//获取图片这个字节流
        ) {
            byte[] buffer = new byte[1024];//缓冲区
            int b = 0;
            while ((b = fis.read(buffer)) != -1) {
                os.write(buffer, 0, b);//每次循环最多输出1024个数据
            }
        } catch (IOException e) {
